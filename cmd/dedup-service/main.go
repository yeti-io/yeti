...
func main() {
	rootCmd := &cobra.Command{
		Use:   "dedup-service",
		Short: "Deduplication Service for data pipeline",
		Long:  "Deduplication Service removes duplicate messages",
		RunE:  serveCmd().RunE,
	}
...
func serveCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "serve",
		Short: "Start the deduplication service",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				earlyLog.Error("Config error: %v", err)
				return err
			}

			log, err := logger.New(cfg.Logging.Level)
			if err != nil {
				earlyLog.Error("Logger error: %v", err)
				return err
			}
			defer log.Sync()
			...
			log.InfowCtx(ctx, "Starting Deduplication Service")
			...
			log.InfowCtx(ctx, "Dedup service running")
